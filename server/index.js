...
app.get("/active", async (req, res, next) => {
  try {
    ...
    console.log(user_id);
    const userMatch = await pool.query("SELECT userid FROM users WHERE userid = $1", [user_id]);
...
app.get("/tasks", async (req, res) => {
  try {
    const allTasks = await pool.query("SELECT * FROM Task ORDER BY ord_no");
...
app.get("/tasks/:target", async (req, res) => {
  try {
    ...
    const task = await pool.query("SELECT * FROM Task WHERE taskId = $1", [target]);
...
app.put("/tasks/:target", async (req, res) => {
  try {
    ...
    const updateTask = await pool.query("UPDATE Task SET taskTitle = $1 WHERE taskId = $2", [tasktitle, target]);
...
app.put("/tasks/completed/:target", async (req, res) => {
  try {
    ...
    const updateTask = await pool.query("UPDATE Task SET completed = NOT completed WHERE taskId = $1", [target]);
    res.json("Todo updated");
  } catch (error) {
    console.error(error);
  }
...
app.delete("/tasks/:target", async (req, res) => {
  try {
    ...
    const deleteTask = await pool.query("DELETE FROM Task WHERE taskId = $1", [target]);
...
app.get("/cards/:user_id", async (req, res) => {
  ...
  try {
    const allCards = await pool.query("SELECT * FROM card WHERE user_id = $1 ORDER BY order_id", [user_id]);
...
app.get("/cards/:target", async (req, res) => {
  try {
    ...
    const card = await pool.query("SELECT * FROM card WHERE cardId = $1", [target]);
...
app.put("/cards/:target", async (req, res) => {
  try {
    ...
    const updateTask = await pool.query("UPDATE card SET cardTitle = $1 WHERE cardId = $2", [cardTitle, target]);
...
app.delete("/cards/:target", async (req, res) => {
  try {
    ...
    const deleteCard = await pool.query("DELETE FROM Card WHERE cardid = $1", [target]);
    const allTasks = await pool.query("DELETE FROM Task WHERE parentid = $1", [target]);
